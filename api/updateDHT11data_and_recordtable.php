<?php
  ...
  if (!empty($_POST)) {
    ...
    $sql = "UPDATE data_main SET temperature = ?, humidity = ?, status_read_sensor_dht11 = ?, time = ?, date = ? WHERE id = ?";
    $q = $pdo->prepare($sql);
    ...
    while ($found_empty == false) {
      ...
      $sql = 'SELECT * FROM data_main WHERE id="' . $id_key . '"';
      $q = $pdo->prepare($sql);
      $q->execute();
    ...
    // This table is also used to store and record the state of the LEDs, the state of the LEDs is controlled from the "home.php" page. 
    // This table is operated with the "INSERT" command, so this table will contain many rows.
		$sql = "INSERT INTO data_main (id,board,temperature,humidity,status_read_sensor_dht11,LED_01,LED_02,time,date) values(?, ?, ?, ?, ?, ?, ?, ?, ?)";
		$q = $pdo->prepare($sql);
